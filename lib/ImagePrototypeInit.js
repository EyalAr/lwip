(function(undefined) {
    ...
    var judges = {
        ...
        dominateColor: decree(defs.args.dominateColor)
    };
    ...
    Image.prototype.dominateColor = function() {
      var that = this,
          ret_pixel;
      judges.dominateColor(
        arguments,
          function(skips){
            ...
            for(var i = 0; i < that.width(); i++){
              for(var j = 0; j < that.height(); j+= skips+1){
                ...
                if( occur === undefined)
                  ...
                  hash.set(key,occur + 1);
              }
            }

            the_key = [...hash].reduce(function(prev,curr){
